...
const RecommendedResult = (props) => {
    ...
    function addFilterSearch(filter) {
        ...
        //     const tags = window.location.href.split("query=")[1].split("!!")
        //     if (!tags.includes(filter)) {
        //         window.location.href = window.location.href.concat("!!".concat(filter))
        //     }
        // } else {
        //     window.location.href = "/search?query=".concat(filter)
        // }
        if (!tags.includes(filter)) {
            setTags([...tags, filter])
        }
    ...
    return (
        <Card className="mt-6 w-80 border-2 p-2 mx-2 mb-5 rounded-md">
            <CardBody>
                ...
                <h1 className="font-bold text-l">Rating: {(avg === 0) ? "N/A" : avg} <strong style={{ color: '#FEBF0F' }}>&#x2605;</strong> </h1>
